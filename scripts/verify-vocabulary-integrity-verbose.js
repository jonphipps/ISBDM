 ...
 * Verbose version that shows complete values.
 * 
 * Usage: node verify-vocabulary-integrity-verbose.js
 */
...
function checkVocabularyFile(filePath) {
  try {
    ...
    const result = {
      ...
      hasIssues: false,
      issues: [],
      allValues: [] // Store all values for verbose output
    };
    ...
    // Store all values for reporting
    result.allValues = conceptsArray.map(c => ({ 
      value: c.value || '', 
      length: c.value ? c.value.length : 0,
      definitionLength: c.definition ? c.definition.length : 0
    }));
    ...
    conceptsArray.forEach((concept, index) => {
      ...
      // Adjusted threshold to 40 to account for legitimate longer values
      if (concept.value && concept.value.length > 40) {
        result.hasIssues = true;
        result.issues.push(`Concept "${concept.value}" has a suspiciously long value (${concept.value.length} chars)`);
      }

      // Check for suspiciously long definition (potential concatenation)
      if (concept.definition && concept.definition.length > 500) {
        result.hasIssues = true;
        result.issues.push(`Concept "${concept.value}" has a suspiciously long definition (${concept.definition.length} chars)`);
      }
 ...
 */
async function main() {
  console.log('Verifying vocabulary file integrity (VERBOSE MODE)...');
  ...
  if (filesWithIssues.length > 0) {
    ...
    filesWithIssues.forEach(file => {
      ...
      if (file.allValues) {
        console.log('\n  All values in this file:');
        file.allValues
          .filter(v => v.length > 30) // Show values longer than 30 chars
          .forEach(v => {
            console.log(`    "${v.value}" (${v.length} chars, definition: ${v.definitionLength} chars)`);
          });
      }
    });
  }
